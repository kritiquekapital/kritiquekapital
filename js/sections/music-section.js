export const musicSection = {
  title: "music",
  kicker: "sets, curation, close listening, and song analysis",
  modules: [
    {
      type: "music-feature",
      entries: [
        {
          id: "this-charming-man",
          label: "this charming man",
          eyebrow: "Everyone But Him",
          title: "The Smiths — This Charming Man",
          dek: "On one person at two distances form the same vanishing point.",
          video: {
            title: "music video",
            src: "https://geo.dailymotion.com/player.html?video=xsk4x",
            aspectRatio: "16 / 9"
          },
          lyrics: {
            title: "lyrics",
            paragraphs: [
               "[Verse 1]",
               "Punctured bicycle on a hillside desolate",
               "Will nature make a man of me yet?",
               "When in this charming car",
               "This charming man",
               "Why pamper life's complexity",
               "When the leather runs smooth on the passenger seat?",
               "I would go out tonight",
               "But I haven't got a stitch to wear",
               "This man said, It's gruesome",
               "That someone so handsome should care",
               "[Chorus]",
               "Ah, a jumped-up pantry boy",
               "Who never knew his place",
               'He said, "Return the ring"',
               "He knows so much about these things",
               "He knows so much about these things",
               "[Verse 2]",
               "I would go out tonight",
               "But I haven't got a stitch to wear",
               'This man said, "Its gruesome',
               'That someone so handsome should care"',
               "Na, na-na, na-na, na-na",
               "This charming man",
               "Na, na-na, na-na, na-na",
               "This charming man",
               "[Chorus]",
               "Ah, a jumped-up pantry boy",
               "Who never knew his place",
               'He said, "Return the ring"',
               "He knows so much about these things",
               "He knows so much about these things",
               "He knows so much about these things",
            ]
          },
          analysisSeries: [
            {
              label: "reading I",
              title: "Everyone But Him",
              paragraphs: [
                "This Charming Man presents itself as a seduction and ends as something far darker–the complete portrait of a self that elected its own erasure one small choice at a time. The charming man and the boy on the hillside are not two people. They are one person at different distances from the same vanishing point. The boy still has a bicycle, a ring, and a body particular enough to be stranded. The charming man has shed all of that–the particularity, the weight, the prior commitments–until what remains is pure function. The encounter between them isn't seduction. It's a person meeting their own future and recognizing it. The desire that moves between them isn't external. It's the self being drawn toward what it is already becoming.",
                "The charming man isn't a villain, isn't a predator in any dramatic sense, is simply what remains when everything that made genuine contact possible has been returned along with the ring. The encounter on the hillside isn't the story. It's one iteration of a loop that has no origin and no end, the charming man himself having once been the boy with the punctured bicycle, shaped by his own hillside, now passing the pattern forward through the only thing he has left–the gravity of the blank center dressed in leather and ease.",
                "The song's genius is structural. The repetition enacts the thesis. The charm that deadens its own meaning, the words that drain across three passes until they're just sound, the smoothness that leaves no friction for anything real to catch on–Morrissey doesn't explain any of this, he just lets the song do what the man does. Consume itself into flatness and keep going.",
                "Underneath it is the oldest human question the floating rock never answers–what do you build around the wanting. This is one answer. Shed everything that complicates it, return every ring, get in every car, keep tasting because the tasting is the only thing left and stopping would mean meeting the blank center with nothing there to meet it. Not suffering. Not even loneliness. Just the mechanism still running in the empty house, moving toward the next hillside, the next briefly luminous thing to draw close and leave a little darker.",
                "That's what the repetition says is lost. Not named, not grieved. Just demonstrated, from outside, to everyone but him."
              ]
            }
          ]
        },
        {
          id: "oh-mama",
          label: "oh mama",
          eyebrow: "Get On In It",
          title: "Milky Chance — Oh Mama",
          dek: "On release, idealization, and what has to be shed for love to survive.",
          video: {
            title: "music video",
            src: "https://geo.dailymotion.com/player.html?video=x9zraaa",
            aspectRatio: "16 / 9"
          },
          lyrics: {
            title: "lyrics",
            paragraphs: [
            "[Verse 1]",
            "Slowly we are falling through the cracks",
            "It just feels like something we regret",
            "Don't know what to do and how to act",
            "Down on my knees, that's how it feels",
            "Lately we've been covered by the past",
            "But don't you think that we are made to last?",
            "Rather than just stumbling in the dark",
            "In the dark, in the dark",
            "[Chorus]",
            "Oh when we get old mama, we'll think it was no drama",
            "Cos then we will know, it was better letting it go",
            "Why couldn't it be just a little more easy to see, darling?",
            "That would've been all, would've been all we needed to know",
            "[Verse 2]",
            "Sun is up the hillside, us being blue-eyed",
            "Sea is on the low tide, trouble on the inside",
            "So far we've been trying to see the light",
            "So hard, missing it was truely bright",
            "Surely right, feelings tight",
            "Talking up the call on my mind, saying",
            "Oh, ma, why is everything so messed up?",
            "Anyway we made it so far",
            "What are we chasing, true love?",
            "How does it feel like",
            "Remedy, so it can only be",
            "As strong as the force of your fantasy",
            "Forever more it's gonna be a mystery",
            "[Chorus]",
            "Oh when we get old mama, we'll think it was no drama",
            "Cos then we will know, it was better letting it go",
            "Why couldn't it be just a little more easy to see, darling?",
            "That would've been all, would've been all we needed to know",
            "[Instrumental]",
            "Mama Drama",
            "[Chorus]",
            "Oh when we get old mama, we'll think it was no drama",
            "Cos then we will know, it was better letting it go",
            "Why couldn't it be just a little more easy to see, darling?",
            "That would've been all, would've been all we needed to know",
            ]
          },
          analysisSeries: [
            {
              label: "reading II",
              title: "Get On In It",
              paragraphs: [
                "\"I think this song basically is about the moment when you start realizing that letting go would have been much better and easier. To get on in the relationship instead of holding on to something which is obviously not going anywhere anymore.\" — Clemens Rehbein",
                "The conventional reading stops at letting go and hears a breakup. But Clemens doesn't say leave. He says get on in. The letting go is not an exit. It is a shed–something released inside the relationship so the relationship itself can move. What isn't going anywhere isn't the person. It's the idea.",
                "\"Oh Mama\" is a song about idealized love as rupture. The fantasy doesn't sustain the relationship–it is what's destroying it. The blue-eyed naivety, the love functioning as remedy, economically–these aren't romantic images, they're an autopsy of a conception of love that was never going to survive contact with an actual person. \"As strong as the force of your fantasy\" is the diagnostic center, clinical where the rest of the song is searching.",
                "The chorus projects forward–when we get old, we'll know it was better letting it go, why couldn't it be easier to see. This is not nostalgia. It is two people inside something still happening, already imagining the hindsight they can't act on yet. Knowledge was never the obstacle. They essentially had it. The fantasy was simply more powerful than the knowing.",
                "And underneath all of it, the address–mama. Not sentimental, not incidental. The thesis. She is who taught him what love actually is, before he knew what it was supposed to look like. Before the idea of it arrived and began crowding out the experience. The one love that required no fantasy to sustain it, that simply was. He reaches toward her not for comfort but for the template–the proof that the thing he's chasing already existed, and that he learned it before he learned to idealize it.",
                "The antidote isn't less love. It's the kind mama already gave him.",
                "Clemens handed the reading in three words. Get on in. The song was never about leaving. It was about what you have to release to stay."
              ]
            }
          ]
        },
        {
          id: "ur-in-my-head-again",
          label: "ur in my head again",
          eyebrow: "Possessed, Not Set Free",
          title: "Funeral — ur in my head again",
          dek: "On intoxication, borrowed strength, and the self it overtakes.",
          video: {
            title: "music video",
            src: "https://www.youtube.com/embed/gEmg7GAe2mE",
            aspectRatio: "16 / 9"
          },
          lyrics: {
            title: "lyrics",
            paragraphs: [
            "[Intro]",
            "(tihs ,hO)",
            "Oh, oh-oh",
            "Oh, oh-oh, Oh, oh-oh",
            "Oh, oh-oh",
            "Oh, oh-oh. Oh, oh-oh",
            "[Verse 1]",
            "You want my best",
            "I can't give that, but I can give you more",
            "You're in my head, you're in my ear",
            "So much worse than before",
            "You say the worst things when I'm high",
            "Try my best to ignore",
            "It's like I hate you, but I love you",
            "My toxic amor",
            "[Chorus]",
            "I'll-I'll-I'll do anything you ask me",
            "Love when you attack me",
            "I get so obsessed with you",
            "I love when you harass me",
            "Don't look in the backseat",
            "Of this fucking taxi",
            "You might see something that you don't want to",
            "Just look past me",
            "[Bridge]",
            "I love, I love, I love, I love, I love, I love",
            "I love, I love, I love, I love, I love, I love",
            "I love, I love, I love, I love, I love, I love",
            "I love, I love, I love",
            "I love the way you can talk me out of anything",
            "You never care if it's bad or if it's good for me",
            "The way this hate sits in my heart, you're my enemy",
            "You never let me take a hold of my identity, ah",
            "[Chorus]",
            "I'll-I'll-I'll do anything you ask me",
            "Love when you attack me",
            "I get so obsessed with you",
            "I love when you harass me",
            "Don't look in the backseat",
            "Of this fucking taxi",
            "You might see something that you don't want to",
            "Just look past me",
            "[Verse 2]",
            "Like you always do",
            "You know all I want is you",
            "Please don't get too close to me",
            "Not what I wanna do",
            "Not what I wanna see",
            "You show me things that I don't need",
            "And I feel so sick when you're all done",
            "But that's just me",
            "[Outro]",
            "That's just me, that's just me, just me, that's just me",
            "I'll-I'll—",
            ]
          },
          analysisSeries: [
            {
              label: "reading III",
              title: "Possessed, Not Set Free",
              paragraphs: [
                "What the song reaches toward is a form of intoxicated strength that feels like power precisely because it overtakes the self entirely. The “you” is whatever arrives sharper, louder, less hesitant, a force that seems to cut through weakness by sheer intensity alone. It is loved not despite its damage but because that damage briefly feels like certainty, command, relief. What the speaker wants is not pain itself. It is the sensation of being amplified. The tragedy is that this power never belongs to him. It speaks in his ear, tells him what to do, and never lets him take hold of an identity outside its grip.",
                "That is why the chorus sounds so warped. “Love when you attack me,” “love when you harass me” are not simply masochistic lines. They describe dependence on a force that feels like strength only because it overrides whatever in the self might resist it. “My toxic amor” is the thesis in miniature: not love of suffering, but love of the thing that makes suffering feel like power.",
                "And the song knows the exchange is fatal. “You never let me take a hold of my identity” is the clearest statement in it. Whatever this force gives him, it gives by replacement. It does not reveal the self. It occupies it.",
                "The final line, “that’s just me,” is where dependence hardens into identity. The voice that seemed separate, invasive, commanding, almost romantic in its hold, is admitted at last as part of the self. Not the self set free, but the self remade in the image of what dominates it. The strength the song chases never makes him more himself. It only teaches him to take what overpowers him as who he is.",
              ]
            }
          ]
        }
      ]
    }
  ]
};
